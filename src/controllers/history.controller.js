...
const insertHistory = async (req, res) => {
  ...
  try {
    ...
    const resultDetails = await historyModel.getModifiedHistory(
      client,
      historyId
    );
    client.release();
...
const deleteHistory = async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const countResult = await historyModel.checkHistory(
      client,
      params.historyId
    );
    ...
    if (count < 2) {
      ...
      await historyModel.deleteDetailHistory(
        client,
        params.historyId,
        query.productId
      );
...
const updateTransactionStatus = async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    await historyModel.updateTransactionStatus(client, params.historyId);
    await historyModel.updateTransactionStatusDetail(client, params.historyId);
    await client.query('COMMIT');
    const result = await historyModel.getModifiedHistory(
      client,
      params.historyId
    );
